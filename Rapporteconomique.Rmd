...
In a nutshell : 
we define library
After opening the data file, we go with the lm function in R to create the linear regression model. With that, we have given details or the summary of the econometric hypothesis. we have the chance to know how the political stability grows each time the inflation, GDP and the market_exchange reach out 1% of growth. (we also present the different Coeff)#
After creating the linear regression model, it's time now to use those data in some graphics. (we use scatterplot first which represents two variables; the political stability and Inflation. There we can see t )
After creating the linear regression model, it's time now to use those data in some graphics
...
```
```{r}
hist(data_r_final$political_stability, xlab ='Inflation', ylab = 'Political stability', main = 'Histogramme Political stability/inflation', col = rainbow(9) )
```
